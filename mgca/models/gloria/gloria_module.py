...
class GLoRIA(LightningModule):
    ''' PyTorch Lightning implementation of GLoRIA
    '''
...
def cli_main():
    ...
    # model args
    parser = GLoRIA.add_model_specific_args(parser)
    ...
    model = GLoRIA(**args.__dict__)

    # get current time
    now = datetime.datetime.now(tz.tzlocal())
    extension = now.strftime("%Y_%m_%d_%H_%M_%S")
    ckpt_dir = os.path.join(
        BASE_DIR, f"../../../data/ckpts/GLoRIA/{extension}")
    ...
    wandb_logger = WandbLogger(
        project="GLoRIA", save_dir=logger_dir, name=extension)
