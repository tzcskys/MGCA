...
class SSLDetector(LightningModule):
    def __init__(self,
        ...
        self.model = ModelMainVit(img_encoder)
        self.yolo_losses = []
        for i in range(3):
...
class ModelMainVit(nn.Module):
    """
    following the ViTDet, use the last feature map to simply upsample or downsample to construct the simple FPN, no lateral connection
    """
    def __init__(self, backbone, is_training=True):
        super(ModelMainVit, self).__init__()
        self.training = is_training
        self.backbone = backbone
        self.anchors = torch.tensor([
            [[116, 90], [156, 198], [373, 326]],
