...
class MIMHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = Transformer(self.decoder_hidden_size, self.decoder_num_layers + 1, self.decoder_num_heads) ## TODO: +1 for what? see m3ae
    ...
    ## this one is for mask along the batch dimension ()
    def forward(self, x, ids_restore, attention_mask_len):
        # embed tokens
        x = self.decoder_embed(x)

        # append mask tokens to sequence
        attention_mask_len = attention_mask_len.cpu().numpy().astype(int)
        for i in range(x.shape[0]):
            if (ids_restore.shape[1] + 1) != attention_mask_len[i]:
                mask_tokens_ = self.mask_token.repeat(1, ids_restore.shape[1] + 1 - attention_mask_len[i], 1).squeeze(0)
                x[i, attention_mask_len[i]:, :] = mask_tokens_ # the first token is cls token, while the attention_mask_len also includes the cls token
        x_ = x[:, 1:, :] # remove the cls token before unshuffle
