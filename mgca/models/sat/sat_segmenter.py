...
def cli_main():
    ...
    if args.base_model == "vit":
        ...
        params = {re.sub('^module.img_encoder.', '', k): v for k, v in state_dict.items()}
        params = {k: v for k, v in params.items() if k in model.state_dict()}
        # there are two fc layers in the downloaded sat model, but the defined resnet50 here only has one fc layer; fc is not used in the segmentation, see the mgca_segmenter.py
        params["fc.bias"] = None
    ...
    ckpt_dir = os.path.join(
        BASE_DIR, f"../../../data/ckpts/sat_segmentation/{extension}")
    ...
    wandb_logger = WandbLogger(
        project="sat_segmentation", save_dir=logger_dir,
