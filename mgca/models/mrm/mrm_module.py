...
class MRM(LightningModule):
    ''' PyTorch Lightning implementation of MRM
    '''
...
def cli_main():
    ...
    # model args
    parser = MRM.add_model_specific_args(parser)
    ...
    model = MRM(**args.__dict__)

    # get current time
    now = datetime.datetime.now(tz.tzlocal())
    extension = now.strftime("%Y_%m_%d_%H_%M_%S")
    ckpt_dir = os.path.join(
        BASE_DIR, f"../../../data/ckpts/MRM/{extension}")
    ...
    wandb_logger = WandbLogger(
        project="MRM", save_dir=logger_dir, name=extension)
